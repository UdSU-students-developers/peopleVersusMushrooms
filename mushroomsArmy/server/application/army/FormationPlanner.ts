...
// Максимальное количество L-стен взрывоморов (3 уровня). После третьей стены
// новые стены не строятся, и армия в режиме обороны не уходит дальше неё.
const MAX_WALL_RINGS_COUNT = 3;
...
export class FormationPlanner {
    ...
    public updateForCounts(
        ...
        // Жёсткий потолок: в режиме обороны армия не уходит дальше последней
        // стены взрывоморов; иначе — стандартный буфер до следующего тригера.
        const maxDefenseD = defenseHold
            ? Math.max(MIN_D, this.lastWallRingIdx * L_STEP)

...
5.1 requestUnits
   Запрос: 
   URL: "requestUnits" (Есть в глобальном конфиге)
   Параметры {
       armyGuid: string;
       unitsType: string; (тип запрашиваемого здания)
       unitsAmount: number;
   }

Ответом будет приходить просто ok, сервис дёрнет вашу ручку, как будет готов создать юнита


5.2 requestBuildings
   Запрос: 
   URL: "requestBuildings" (Есть в глобальном конфиге)
   Параметры {
       armyGuid: string;
...
Ответом будет приходить просто ok, сервис дёрнет вашу ручку, как будет готов создать здание

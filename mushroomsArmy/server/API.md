...
| 2.6 | SPAWN_UNIT | Заспавнить нового юнита в армию |
...
Заспавнить нового юнита в указанную армию. Вызывается сервисом `mushroomsEconomy` когда инкубатор вырастил юнита.
...
```
{
    ...
    x:        number; - координата X спавна (конечное число)
    y:        number; - координата Y спавна (конечное число)
}
```
...
* `242` - не переданы все параметры, `type` не из допустимых значений, `x`/`y` не конечные числа, армия не найдена, тайл в точке спавна занят водой (`1`) или туманом (`null`), координаты вне границ карты

#### 4.2.7 SPAWN_BUILDING

`POST /spawnBuilding`

Заспавнить новое здание в указанную армию. 
...
* `242` - не переданы все параметры, `type` не из допустимых значений, `x`/`y` не конечные числа, армия не найдена, тайл в точке спавна должен быть 0 (не вода, не горы, не туман), для `sporovaya_bashnya` — дополнительная проверка 2×2 свободных тайлов, координаты должны быть в пределах границ карты
